#!/bin/bash
# from https://askubuntu.com/a/1321904
...
sink_name=$(pacmd list-sinks | grep -P "index:|\s+device\.description\s+=" | grep -A 1 -C 0  -P "\*\s+index" | tail -n 1 | sed -n 's/^.*"\(.*\)".*$/\1/p')

notify-send -t 150 -c device -i multimedia-volume-control "Output changed" "Output set to $sink_name"
